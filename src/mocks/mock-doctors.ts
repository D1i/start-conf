...
export function generateMockDoctor(): Doctor {
  const firstName = pickRandom(firstNamesMock);
  const lastName = pickRandom(lastNamesMock);
  const selfInfo = pickRandom(selfInfosMock);
